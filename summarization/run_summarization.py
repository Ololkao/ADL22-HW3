...
class DataTrainingArguments:
    ...
    do_sample: bool = field(
        ...
        metadata={
            "help": (
                "Whether or not to use sampling ; use greedy decoding otherwise. This argument will be passed to ``model.generate``, "
...
def main():
    ...
    trainer = Seq2SeqTrainer(
        ...
        compute_metrics=compute_metrics if training_args.predict_with_generate and not training_args.do_predict \
                        else None,
    )
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                #     writer.write("\n".join(predictions))
                output_prediction_file = data_args.output_file if data_args.output_file is not None \
                                    else os.path.join(training_args.output_dir, "predictions.jsonl")
                with jsonlines.open(output_prediction_file, 'w') as writer:
                    for i, title in zip(raw_datasets["test"]['id'], predictions):
                        writer.write({
                            "title": title,
                            "id": i
                        })
